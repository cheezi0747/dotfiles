...
render_bar_item() {
	if [ "$NOTIFICATIONS" = "[]" ]; then
		args+=(--set github.bell icon="$BELL" --set github.label label="0")
	else
		args+=(--set github.bell icon="$BELL_DOT" --set github.label label="$COUNT")
	fi
}
...
update() {
	...
	PREV_COUNT=$(sketchybar --query github.label | jq -r .label.value)
	...
	if [ "$COUNT" -gt "$PREV_COUNT" ] 2>/dev/null || [ "$SENDER" = "forced" ]; then
		sketchybar --animate tanh 15 --set github.label label.y_offset=5 label.y_offset=0
	fi
}
